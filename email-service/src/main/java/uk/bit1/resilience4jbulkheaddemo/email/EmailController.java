...
class EmailController {
    ...
    EmailController(@Value("${demo.email-service.processing-delay:1s}") Duration processingDelay) {
        this.processingDelay = processingDelay;
        log.info("Setting processing delay to *** {} ***", processingDelay.toMillis());
    }

    @PostMapping
    @ResponseStatus(HttpStatus.ACCEPTED)
    Map<String, Object> send(@RequestBody EmailNotificationRequest request) throws InterruptedException {
        log.info("email-service processing order orderId={} delayMs={}", request.orderId(), processingDelay.toMillis());
